...
## Approved live-progress and report follow-up — 2026-09-17

- Added default one-second per-device progress on stderr, with mode, phase, received/processed/ACKed counts, queue size, drops and errors. `--progress-interval 0` disables progress; the library remains quiet by default. Final stdout remains one JSON object.
- Added optional `--report PATH`. An exclusive incomplete reservation is created before input initialization, existing or unusable destinations are refused, and normal completion saves the same final JSON as stdout. Finalization failures produce a non-clean result and nonzero exit. Metadata includes UTC timing, mode, requested duration/rate/session and the available Git HEAD revision.
- Final regression: `python -m pytest laptop/tests tests phone/tests -q` — **285 passed, 3 skipped in 103.16 s**. Tests cover live output before completion, file/stdout equality, destination refusal, incomplete reports, write failures, progress failure cleanup and visible disconnect accounting.
- A ten-second local run used the actual CLI with two synthetic sources, a real TLS Week7Server and a desktop result subscriber. Both devices generated, received and acknowledged **104 samples**; all **208 results** reached the subscriber, with zero sequence anomalies, duplicates, rejected input or result drops. Progress lines appeared for both devices and the saved report matched stdout. This is software evidence; it does not establish physical BLE or iPhone acceptance.
- Independent review passed with no remaining Important/Critical findings. The runbook includes the new flags and explains provisional live counts versus the final source reconciliation. Firmware, sensor/result wire formats and the iPhone app were unchanged by this follow-up.
