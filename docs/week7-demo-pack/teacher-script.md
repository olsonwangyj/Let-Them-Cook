...
“Separate live captures now include USB-only power loss and the physical RESET button. USB recovery yielded 2,715 matching ACKs/results over 300.422 seconds; RESET yielded 5,832 over 600.422 seconds. Each showed a new boot and authenticated BLE recovery. Both remain `passed=false` because the interruptions broke the clean-run criteria. A separate final check passed with 100 matches in 13.797 seconds and zero checked errors or drops. We independently matched the measured IDs to the Ultra96 acceptance log.”

“The remaining final-path work is a real Phone run and the teammate's visualizer integration, including its ten-minute run and lifecycle behavior. Android is the baseline; the iPhone/iSH procedure remains unverified. The portable C# core has been compiled and tested; the Unity component and actual app still need their target environment. The demonstrated scope is one ESP, dummy values, deterministic results and the measured communication paths.”
