...
Before speaking, complete the [operator checklist](README.md). Open [the brief](teacher-brief.html) or [printable PDF](teacher-brief.pdf), [packet walkthrough](packet-walkthrough.md), the [full iPhone audit](D:/LetThemCook-builds/iphone-live-20260907/phone100-20260908-185338/phone-full-audit.json) and the saved desktop 600-second evidence. The pack also carries [a recorded 100-packet desktop example](recorded-demo100.jsonl) with [provenance and hashes](evidence-index.json); keep its desktop label when presenting it. Keep password and serial-passkey terminals off the shared screen.
...
“We have now received 100 matching results on the actual iPhone using its own SSH/TLS connection and Python JSON display. There was one startup read-timeout retry before the first result; the log records no further reconnect during those 100 results. The teammate's visualizer integration is still separate work.”
...
Show the sender summary, exact-ID audit and original Phone status together. The verified iPhone IDs are `1:2375739948:0` through `:99`; the sender ran for 14.078 seconds. Original Phone log lines 1–3 contain subscription, `TimeoutError` and re-subscription; lines 4–103 contain the 100 ordered results; the final line reports `received=100,reconnects=1`. Receiver exit is 0. Do not call this a zero-reconnect pass or treat the sender duration as a measured Phone duration. For a fresh run, report its actual outcome and retain any failure.
...
**3:50–5:00 — Show physical recovery and state the next gate.**
...
“The actual iPhone 100-message delivery is verified, including its startup retry. A Phone ten-minute run, lifecycle recovery and the teammate's visualizer remain to be tested. The portable C# core has been compiled and tested; the Unity component and actual app still need their target environment. Any new startup-grace change also needs an on-Phone rerun before we call it a verified fix.”
...
| Remaining Phone and visualizer requirements | [Phone guide](../week7-phone-runbook.md), [iPhone quickstart](../week7-iphone-quickstart.md), [original goal](../week7-development-plan.md), [current acceptance table](../week7-continuation-report-2026-09-07.md#acceptance-status-and-remaining-work) | Actual iPhone Python delivery is verified for 100 messages with one startup retry. Phone 600-second/lifecycle tests, a zero-reconnect rerun, teammate visualizer and full Gate M remain pending. Portable C# core tested; supplied Unity component uncompiled here. New startup-grace behavior has not yet been physically verified. |
...
**What does the startup timeout mean?** The original full log shows one read timeout after subscription and before the first result, then 100 matching results with no further logged reconnect. It proves a retry occurred, not the precise upstream cause. The derived result-only JSONL omits status lines, so show it together with the original log and full audit. A subsequent software startup-grace change is not yet a physically verified fix.
...
**Does this finish the original Week 7 goal?** The actual iPhone now has 100 matching dummy results through its independent connection, with the startup retry retained. Phone endurance/lifecycle tests and the teammate's visualizer remain outstanding. The official written course rubric was not available, so this pack makes no grading or rubric-acceptance claim.
